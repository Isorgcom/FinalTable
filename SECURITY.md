...
- **Admin password** compared with `timingSafeEqual` and never sent to a client, logged or put in state. A password set from the Admin page is stored as a scrypt hash with a per-server salt in `data/settings.json`, never in the clear; the one in `ADMIN_PASSWORD` is what a server starts with, and is compared as it stands because the environment is where it already lives in the clear. Changing it requires the current one and signs out every other admin session. Five wrong answers per socket, each costing a delay, and no password configured at all means the surface does not exist rather than existing behind a default
...
- **The admin's list** (`adminListTournaments`) is the one place an unlisted game and its code are shown to someone not in it: behind the admin password, answered only on the socket that unlocked it, never broadcast. The admin runs the machine and holds `data/tournaments.json` already, so this is a window, not a door
