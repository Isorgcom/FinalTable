...
- **The operator's list** (`adminListTournaments`) is the one place an unlisted game and its code are shown to someone not in it: behind the operator password, answered only on the socket that unlocked it, never broadcast. The operator runs the machine and holds `data/tournaments.json` already, so this is a window, not a door
