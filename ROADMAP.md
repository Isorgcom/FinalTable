...
is the administrator's own integration key - and all three were closed in
0.26.1 anyway: a webhook may only be sent to the paired GameNight's origin
(or one named in `WEBHOOK_ORIGINS`), checked when the game is made and again
when it is read back from disk, with the failure reason coarsened to two
states in the API's answer while the admin Log keeps the detail; the key
reads and drives only the games it made, a browser-made game answering the
way a wrong id does; and delivery ids are written down so they never repeat
after a prune, which a receiver deduping on them depends on.
